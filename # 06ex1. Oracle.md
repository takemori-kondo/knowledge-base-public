...
________________________________________
### 1.0. WindowsとLinuxのデフォルト構成の違いや、想定される構成

設定                |Windows                 |Linux
--------------------|------------------------|-----------------------------
Namingメソッド      |TNSNAMES, EZCONNECT     |TNSNAMES, EZCONNECT, HOSTNAME
Network Security認証|NTS                     |-
hosts               |自分のホスト名はない想定|自分のホスト名がある想定
OSユーザのoracle    |ログイン不可            |ログイン可能
想定される操作      |通常ユーザから起動操作  |oracleユーザから起動操作

- 以下の状態だと、listener.oraを構成しなくてもリスナが起動できる（linuxの場合、このようになってる）
    - hostsに「127.0.0.1 自分のホスト名」が存在している
    - OSユーザoracleからlsnrctl startする
...
[ORACLE_BASE]/fast_recovery_area           |普段は考慮不要。リカバリ用のトランザクションログが保存される場所
...
※2 Oracle12c以降のCDBおよびPDBの場合は以下の通り
