...
1. Unity6Playerやライブラリ開発はStandard2.1推奨
2. Unity6Editorは.NET Framework互換
...
```text
特別な理由がない限り、非推奨な文法や機能

- C#1.0：throw ex
- C#1.0：非ジェネリックコレクション全般
- C#1.0：Delegateの自作
- C#1.0：event修飾子
- C#2.0：匿名メソッド
- C#2.0：partial (自動生成されるコードを除く)
- C#4.0：dynamic
- C#6.0：using static
- C#7.0：refローカル変数
- C#7.2：条件演算子ref
...
- Strippingされた型へのReflection、Serialization
- when例外フィルター
- 実質的な動的IL
- dynamic
- OS依存機能
...
Unity    |C#
---------|-----
~2020    |8.0(fw4.6相当)。ただし一部機能は使えない。.NET Standard 2.1も8.0
...
8.0 で利用可能なnullableアノテーション

- AllowNull
- DisallowNull
- return/out: MaybeNull
- return/out: NotNull
- return/out: MaybeNullWhen(bool)
- return/out: NotNullWhen(bool)
- return/out: NotNullIfNotNull("parameterName")
- DoesNotReturn
- DoesNotReturnIf(bool)

9.0 あるいはUnity2022以降で使用可能なアノテーション。.NET Standard2.1は不可能

- MemberNotNull("MemberName")
- MemberNotNullWhen(bool, "MemberName")
...
- null許容参照型を拾いつつ大半の別文法はヒットしない
    - 直前が空白ではなく、直後が?でも.でもない
    - (?<!\s)\?(?!\?|\.)
- null forgivingを拾いつつ大半の別文法はヒットしない(コメントや空白が挟まるケースを除く)
    - 単語])}いずれか直後に!、直後が=でない
    - (?:\b\w+|\]|\)|\})!(?!=)
