...
3. サンプルは、開発中とUPM側の挙動を考慮すると、Samples~ + Samplesシンボリックリンクが現実的(サンプル機能リリース当初からの推奨)
    - https://discussions.unity.com/t/samples-in-packages-manual-setup/730846
    - 補足：このプラクティスを守らない場合に起きる問題
        - 推奨フォルダ構成通りにすると、利用者が常にSamplesをダウンロードする必要がある
        - 利用者から見ると、常に含まれるSamplesが読み取り専用で弄れない
        - Sampleダウンロード機能に対応させてSampleをダウンロードさせると、重複エラーが出てしまう
    - 注意：シンボリックリンクは複数の問題を抱える
        - VisualStudio上でのGitステータスが正常に表示されなくなる
        - Search Window による検索が、機能しない場合がある
